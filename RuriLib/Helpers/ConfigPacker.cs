...
public static class ConfigPacker
{
    ...
    public static async Task<byte[]> PackAsync(IEnumerable<Config> configs)
    {
        ...
        using (var archive = new ZipArchive(packageStream, ZipArchiveMode.Create, false))
        {
            foreach (var config in configs)
            {
                ...
                if (fileNames.TryGetValue(fileName, out var index))
                {
    ...
    public static Task<Config> UnpackAsync(Stream stream)
    {
        ...
        using (var archive = new ZipArchive(stream, ZipArchiveMode.Read, false))
        {
            ...
            try
            {
                config.Metadata = JsonConvert.DeserializeObject<ConfigMetadata>(
                    ReadStringFromZipEntry(archive, "metadata.json"),
                    jsonSettings) ?? throw new InvalidDataException("Could not deserialize metadata.json.");
            }
            catch
            {
            ...
            try
            {
                config.Settings = JsonConvert.DeserializeObject<ConfigSettings>(
                    ReadStringFromZipEntry(archive, "settings.json"),
                    jsonSettings) ?? throw new InvalidDataException("Could not deserialize settings.json.");
            }
            catch
            {
    ...
    private static byte[] ReadBytesFromZipEntry(ZipArchive archive, string path, bool essential = true)
    {
        ...
        if (entry is null)
        {
            ...
            throw new FileNotFoundException("File not found inside the opk archive", path);
        }
